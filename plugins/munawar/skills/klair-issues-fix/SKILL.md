...
- **If NO `QC_BOT_COMMENTS:` lines are found:** STOP immediately. Do NOT proceed to parsing or launching agents. Tell the user:
  ...
  > "This file has not been QC-annotated yet. Run `/m:klair-issues-eval <file>` first to have each issue independently validated before fixing. If you want to proceed anyway without QC, re-run with explicit instruction to skip the QC gate."
